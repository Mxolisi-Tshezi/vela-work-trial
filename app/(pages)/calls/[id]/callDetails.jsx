...
// import { 
//   findPlaceholdersToReveal, 
//   getOrgPlaceholders, 
//   revealInfo, 
//   getRedactedTimestamps, 
//   getMuteTimestamps 
// } from "./page";
...
//       // Define getOrgPlaceholders locally if import fails
//       const getLocalOrgPlaceholders = (redactionSettings) => {
//         const redactable = {
...
//       // Use imported function or local fallback
//       const orgPlaceholders = (typeof getOrgPlaceholders === 'function') 
//         ? getOrgPlaceholders(orgRedactionSettings) 
//         : getLocalOrgPlaceholders(orgRedactionSettings);

//       // Generate better mock redacted timestamps by actually parsing the text
//       const mockRedactedTimestamps = [];
//       call.segments.forEach(segment => {
//         if (segment.redaction && segment.words && segment.words.length > 0) {
//           // Extract redacted placeholders from the text
//           const regex = /<[^>]+>/g;
//           const redactedMatches = segment.redaction.match(regex) || [];

//           if (redactedMatches.length > 0) {
//             console.log("Found redacted matches in segment:", redactedMatches);

//             // Find the positions of these placeholders in the words array
//             const redactedWords = [];
//             let wordIndex = 0;
//             const words = segment.redaction.split(/\s+/);

//             words.forEach((word, index) => {
//               if (redactedMatches.some(match => word.includes(match))) {
//                 // This word contains a redaction placeholder
//                 redactedWords.push(index);
//               }
//             });

//             console.log("Redacted word positions:", redactedWords);

//             // Map these positions to actual timestamps in the words array
//             redactedWords.forEach(pos => {
//               if (segment.words[pos] && segment.words[pos].start !== undefined && segment.words[pos].end !== undefined) {
//                 // Create a redacted timestamp with a buffer
//                 const startTime = segment.words[pos].start * 1000 + segment.start - 200;
//                 const endTime = segment.words[pos].end * 1000 + segment.start + 200;

//                 mockRedactedTimestamps.push({
//                   word: segment.words[pos].word,
//                   start: startTime,
//                   end: endTime
//                 });

//                 console.log(`Added redacted timestamp for word at position ${pos}:`, startTime, endTime);
//               }
//             });

//             // If we couldn't find exact matches, just mark the first few words as a fallback
//             if (redactedWords.length === 0 && segment.words.length > 0) {
//               for (let i = 0; i < Math.min(2, segment.words.length); i++) {
//                 if (segment.words[i].start !== undefined && segment.words[i].end !== undefined) {
//                   mockRedactedTimestamps.push({
//                     word: segment.words[i].word,
//                     start: segment.words[i].start * 1000 + segment.start,
//                     end: segment.words[i].end * 1000 + segment.start
//                   });
//                 }
//               }
//             }
...
//       console.log("Generated redacted timestamps:", mockRedactedTimestamps);
//       setRedactedTimestamps(mockRedactedTimestamps);
//     }
//   }, [call]);
